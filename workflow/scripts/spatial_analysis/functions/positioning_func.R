...
# chunk_vector splits a vector into groups of at most chunk_size elements for parallel dispatch
chunk_vector <- function(v, chunk_size) {return(split(v, ceiling(seq_along(v) / chunk_size)))}

# h-index of a UMI count vector: largest h such that at least h beads have ≥h UMIs
# used as a robust measure of spatial cluster quality that is resistant to a few high-count outliers
...
# grid search over eps × minPts to maximize the fraction of cells with exactly one DBSCAN cluster
# the loop expands the minPts search range if the optimum is still at the upper edge (p >= 0.9),
# ensuring we don't stop before the true optimum is found; at most 26 expansions are attempted
# furrr::future_map dispatches parameter evaluations in parallel across ncores workers
  ...
  for (k in 0:26) {
    ...
    params$pct = furrr::future_map(row_lists, function(v) {
      map_dbl(v, function(i) {
        # count cells where max cluster label == 1, meaning exactly one cluster was found
    ...
    # p is the relative position of the best minPts in the current search range [0,1]
    # if the optimum is in the top 10% of the range, expand and keep searching
...
# compute a weighted centroid for each cell using only beads in the dominant DBSCAN cluster (cluster==1)
# cells with 0 or 2+ clusters are left unplaced (x_um = y_um = NA)
# SNR = UMI in cluster 1 / total UMI, measuring how concentrated the signal is at the placement location
...
kde <- function(df, bw, radius) {
  ...
  # cells with only one bead are assigned its location directly with d2=0 (no ambiguity)
  if(nrow(df) == 1) {
    # field names must match res's initialization (sumi/sbeads, not umi/beads) and the multi-bead
    # branch below; the old umi/beads names silently created spurious fields and left sumi/sbeads NA
    res[c("x_um", "y_um", "d1", "d2", "sumi", "sbeads", "r", "h")] = c(df$x_um, df$y_um, df$umi, 0, df$umi, 1, 1, 1)
    return(res)
  }
  ...
  df$density = exp(-xmu^2/bw) %>% sweep(MARGIN=1, STATS=df$umi, FUN="*") %>% colSums

  # find the highest-density bead and collect all beads within radius of it (first peak)
  rowmax = df %>% {.[which.max(.$density),]}
  df$near = (df$x_um-rowmax$x_um)^2 + (df$y_um-rowmax$y_um)^2 < radius^2
  # find the second-highest-density bead among beads outside the first peak's radius
  ...
  # record centroid of each peak; r and h are quality metrics for the dominant cluster
  ...
  res[c("d1","d2","sumi","sbeads","r","h")] = c(rowmax$density,
                                                ...
                                                sum(sdf$umi),
                                                nrow(sdf),
                                                r=sum(sdf$umi)/max(sdf$umi),  # ratio of cluster UMI to peak bead UMI
                                                h=h_index(sdf$umi)
  )
  return(res)
}
...
sample_bead_plots <- function(data.list, coords) {
  plot.sb <- function(cb_i) {
    ...
    plot <- ggplot() + coord_fixed(ratio=1 ,xlim=xlims, ylim=ylims) + theme_void() +
      geom_point(data=subdf, mapping=aes(x=x_um, y=y_um, col=umi), size=1, shape=16)+
      geom_point(aes(x=row$x_um_kde, y=row$y_um_kde), color="red", shape=5, size=2) + 
  ...
  # sample(x, size) treats a length-1 numeric x as the population 1:x rather than the single
  # value to sample -- guard against that footgun when exactly one cell qualifies
  safe_sample <- function(x, size) {if (length(x) == 1) x else sample(x, size, replace=F)}

  if(len(list0) > 0) {
    p1 = plot_grid(ggdraw()+draw_label("DBSCAN=0"),
                   list0 %>% safe_sample(min(12,len(list0))) %>% map(plot.sb) %>% {plot_grid(plotlist=.,ncol=3)}, ncol=1, rel_heights=c(0.1,2))
  } else {p1 = gdraw("No DBSCAN = 0")}
  if(len(list1) > 0) {
    p2 = plot_grid(ggdraw()+draw_label("DBSCAN=1"),
                   list1 %>% safe_sample(min(12,len(list1))) %>% map(plot.sb) %>% {plot_grid(plotlist=.,ncol=3)}, ncol=1, rel_heights=c(0.1,2))
  } else {p2 = gdraw("No DBSCAN = 1")}
  if(len(list2) > 0) {
    p3 = plot_grid(ggdraw()+draw_label("DBSCAN=2"),
                   list2 %>% safe_sample(min(12,len(list2))) %>% map(plot.sb) %>% {plot_grid(plotlist=.,ncol=3)}, ncol=1, rel_heights=c(0.1,2))
